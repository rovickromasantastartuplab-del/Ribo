...
// import { hasPermission } from '@/utils/authorization'; // Copied inline or stubbed
import { Link } from 'react-router-dom';
import { useTranslation } from 'react-i18next';

// Inline mock for hasPermission if not available yet
const hasPermission = (permissions, permission) => {
    // For now, assume admin access or check if permission logic exists
    // If permissions array is provided, check it.
        ...
        return (
            <div className="flex items-center justify-end space-x-2">
                {actions.map((action, index) => {
                    ...
                    const IconComponent = LucidIcons[action.icon];
                            ...
                            <TooltipProvider key={index}>
                                <Tooltip>
                                    <TooltipTrigger asChild>
                                        <Link to={href} target={action.openInNewTab ? '_blank' : undefined}>
                                            <Button
                                                variant="ghost"
                                                size="icon"
