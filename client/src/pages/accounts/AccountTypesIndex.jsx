...
export default function AccountTypesIndex() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const response = await axios.get('/api/account-config/types', { params });
            ...
            toast.error(t('Failed to load account types'));
            setData([]);
        } finally {
            setLoading(false);
        }
    ...
    const handleFormSubmit = async (formData) => {
        try {
            if (formMode === 'create') {
                ...
                await axios.post('/api/account-config/types', formData);
                toast.success(t('Account type created successfully'));
            } else if (formMode === 'edit') {
                toast.loading(t('Updating account type...'));
                await axios.put(`/api/account-config/types/${currentItem.id}`, formData);
                toast.success(t('Account type updated successfully'));
            }
    ...
    const handleDeleteConfirm = async () => {
        try {
            ...
            await axios.delete(`/api/account-config/types/${currentItem.id}`);
            toast.success(t('Account type deleted successfully'));
    ...
    const handleToggleStatus = async (item) => {
        try {
            ...
            // Assuming the toggle endpoint follows pattern or using PUT update
            // Based on other files, often reused update for toggle or specific route
            // Checking backend routes: accountConfigRoutes doesn't explicitly show toggle, usually handled via update
            // But legacy code called `account-types.toggle-status`. 
            // I'll try standard PUT update first as it's safer if specific route isn't obvious in summary.
            // Actually, checking legacy code calls `route('account-types.toggle-status', ...)`
            // I'll check if I missed a route or if I should just update the status field.
            // The `accountConfigRoutes.js` file showed `updateAccountType` on PUT /types/:id.
            // I will use that.
            await axios.put(`/api/account-config/types/${item.id}`, { ...item, status: newStatus });
    ...
    return (
        ...
        >
            ...
            <CrudFormModal
                ...
                title={formMode === 'create' ? t('Add New Account Type') : t('Edit Account Type')}
                mode={formMode}
            />
