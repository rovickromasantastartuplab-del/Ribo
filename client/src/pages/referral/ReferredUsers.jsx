...
// import { Pagination } from '@/components/ui/pagination'; // Need to check if we have this or if I need to mock it/use a simple one
            ...
            data: [
                {
                    ...
                    plan: { id: 1, name: "Pro Plan", price: 29 },
                    plan_orders: [{ id: 101, billing_cycle: "monthly", final_price: 29 }],
                    referrals: [{ id: 201, amount: 2.9, commission_percentage: 10, created_at: "2023-10-02" }]
                },
                {
                    id: 2,
                    name: "Bob Smith",
...
export default function ReferredUsers() {
    ...
    // Mock Data
    const { referredUsers, userType, currencySymbol } = useReferredUsersData();

    const formatDate = (dateString) => {
        return formatDistanceToNow(new Date(dateString), { addSuffix: true });
    };

    const getTotalCommission = (user) => {
        return user.referrals?.reduce((total, referral) => total + (Number(referral.amount) || 0), 0) || 0;
    };

    const getPlanDisplayInfo = (user) => {
                                                ...
                                                <div>
                                                    ...
                                                    <div className="flex items-center space-x-2 mt-1">
                                                        ...
                                                        <span className="text-xs text-muted-foreground">
                                                            {t('Registered')} {formatDateTime(user.created_at)}
                                                        </span>
                                                    </div>
                                                </div>
                                            </div>
                                                        ...
                                                        return planInfo ? (
                                                            <div>
                                                                ...
                                                                <p className="text-sm text-muted-foreground">
                                                                    {formatCurrency(planInfo.price)}/{t(planInfo.cycle)}
                                                                </p>
                                                            </div>
                                                        ) : (
                                                ...
                                                {getTotalCommission(user) > 0 && (
                                                    <div className="text-right">
                                                        <div className="text-sm font-medium text-green-600">
                                                            +{formatCurrency(getTotalCommission(user))}
                                                        </div>
