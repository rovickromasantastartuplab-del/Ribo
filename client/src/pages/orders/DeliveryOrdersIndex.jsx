...
import { mockDeliveryOrders, mockProducts } from '@/data/mockCrmData';
import { formatCurrency, formatDateTime } from '@/lib/utils';

export default function DeliveryOrdersIndex() {
    const { t } = useTranslation();

    const [data, setData] = useState(mockDeliveryOrders);
    ...
    const columns = [
        ...
        { key: 'shipping_address', label: t('Address') },
        {
            key: 'status',
            label: t('Status'),
            render: (v) => (
                <span className={`inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ring-1 ring-inset ${v === 'delivered' ? 'bg-green-50 text-green-700 ring-green-600/20' : 'bg-blue-50 text-blue-700 ring-blue-600/20'}`}>
    ...
    return (
        ...
        >
            ...
            <CrudFormModal
                ...
                formConfig={{
                    fields: [
                        { name: 'delivery_address', label: t('Delivery Address'), type: 'textarea', required: true },
                        { name: 'delivery_date', label: t('Delivery Date'), type: 'date', required: true },
